...
from PyQt6.QtWidgets import QApplication, QStatusBar, QWidget, QLabel, QLineEdit, QPushButton, QProgressBar, QComboBox, \
    ...
    def run(self):
        try:
            ...
            self.worker_response.emit((
                ...
                [f'{res.resolution} - {round(res.filesize / 1.049e+6, 1)}MB' for res in
    ...
    def run(self):
        ...
        try:
            ...
            # video request
                ...
                self.yt.streams.filter(progressive=True, file_extension='mp4').get_by_resolution(
                    self.download_type).download(output_path=self.path, filename_prefix=f'[{self.download_type}] ')
        except:
            # emitting the error signal
